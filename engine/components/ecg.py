...
class ECG:
    ...
    def updateECG(self):
        ...
        if self._pq_time_counter > self._model['components']['ecg'].pq_time:
        ...
        if self._qrs_time_counter > self._model['components']['ecg'].qrs_time:
        ...
        self._sa_node_counter += self._model['modeling_stepsize']
        # increase the pq time counter if pq time is running
        if self._pq_running:
            self._pq_time_counter += self._model['modeling_stepsize']
        ...
        if self._qrs_running:
            self._qrs_time_counter += self._model['modeling_stepsize']
        ...
        if self._qt_running:
            self._qt_time_counter += self._model['modeling_stepsize']
